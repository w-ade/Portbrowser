...
**Current release:** v0.1.0 (2026-09-23), source-only, with the site live at [portbrowser.com](https://www.portbrowser.com).

The path from here to a signed, notarized download anyone can install. Cards use `###`; sections group them by when they matter.

## Now

### PB-025 · Stamp v0.1.0 into the app and tag it
- **Status:** open
- **Size:** S
- **Tags:** release, metadata
- **Why:** v0.1.0 is announced on the site, but the app bundle carries no version, so the About panel is blank, and the repo has no tag to point at.
- **Spec:** add `CFBundleShortVersionString` (0.1.0) and `CFBundleVersion` to the Info.plist in `app/script/build_and_run.sh`, sourced from one place the site can also read. Tag `v0.1.0` and write short release notes for a source-only GitHub release.
- **Done means:** About Portbrowser shows 0.1.0, `git tag` lists v0.1.0, and the site and app read the same version.
...
## Release track (to a signed download)
...
- **Status:** in progress (blocked on: PB-008 for the asset catalog)
- **Size:** M
- **Tags:** brand, metadata
...
- **So far:** bundle ID `dev.wade.portbrowser`, the Soft depth `.icns`, and an About menu item. Still missing: version and build numbers (PB-025), copyright, category, and an asset-catalog icon.
...
### PB-020 · Publish the first signed download
...
- **Note:** v0.1.0 launched as source-only. This card covers the first release a non-developer can install.
...
### PB-026 · Custom viewport dimensions and landscape
- **Status:** open
- **Size:** M
- **Tags:** presets, viewport
- **Why:** the 11 presets cover current iPhones, but older devices, arbitrary breakpoints, and landscape still need a code change. `ContentView` already has an unused `isLandscape` state.
- **Spec:** add a custom width × height entry to the device menu and a rotate action, keeping window sizing deterministic.
- **Done means:** any logical size and either orientation can be previewed without a rebuild.
...
- **Outcome:** the site is Portbrowser-branded and live at www.portbrowser.com (the apex redirects there), built from `site/` in this repo.
...
- **Outcome:** the name is Portbrowser, and the repo, executable, bundle, and domain all match. Pitch: "a native macOS app for previewing websites at true iPhone dimensions." Distribution is direct download for now; see the Release track.
...
- **Outcome:** 11 iPhone presets (16, 17, and 18 series, Air, 16e/17e) in the toolbar device menu. Custom dimensions did not ship; that is PB-026.
...
- **Superseded:** windows now open at 100% device size (iPhone 18 Pro, 402 × 874) and scale down only to fit the display.
...
- **Note:** windows now remember their position instead of always centering.
...
- **Superseded:** the utility strip was replaced by the Liquid Glass Safari toolbar floating over the page.
